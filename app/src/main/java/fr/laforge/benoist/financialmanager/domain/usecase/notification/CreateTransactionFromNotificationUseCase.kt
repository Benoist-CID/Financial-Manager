...
class CreateTransactionFromNotificationUseCaseImpl(
    ...
    override suspend fun invoke(notificationTitle: String, notificationMessage: String) =
        withContext(dispatcher) {
            // Checks if the message contains a transaction
            if (notificationHelper.isTransaction(notificationMessage = notificationMessage) == Result.success(
                    true
                )
            ) {
                // Creates a transaction from message
                val transaction = notificationHelper.parseNotificationMessage(
